...
In order not to disappoint users, the accuracy of the model needs to be at least 80%. To achieve this the model needs to be trained with more pictures and the layers on top of the VGG16 model might need adaptation. For the local model training the jupiter notebook containing the model could use Goolge Colab GPU resources. However for future releases the manual training process should be transformed to a regular monitored automated training including more bird species.

The performance goal should be set to a maximum of 7 seconds for the prediction part so the user does not get impatient in waiting. To support this code refactoring and a review of several free cloud offers need to be performed.
